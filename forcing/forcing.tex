...
\author{Timothy Y. Chow} %typeset by iza :P
\date{}
\begin{document}
\maketitle
...
The following condition on $G$ solves this problem. Define a subset $D$ of $P$ to be ``dense'' if for every $p$ in $P$ there exists $q$ in $D$ such that $q \leq p$. A filter $G$ is ``generic'' if it intersects every dense subset.
...
\begin{ffact}
Fix a particular generic filter $G$ of $P$. Then for any $\phi$ that is true in $\mathcal M[G]$, there exists $r$ in $G$ such that $r \Vdash \phi$.
\end{ffact}
...
\begin{ffact}
$\Vdash$ is definable in $\mathcal M$.
\end{ffact}
